...
const myStyle = {
  ...
  const Cards = () => {
    ...
    const goDetils=(courseId,courseName)=>{
  
        navigate(`/course?courseId=${courseId}&courseName=${courseName}`);
    ...
    return (
      <div className="flex items-center">
        <div className="container mx-auto">
          {/* card grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 py-10 text-left mx-5 xl:mx-10">
          {courses ? (
  courses.map((item) => (
    <div
      className="border rounded-lg hover:drop-shadow-md overflow-hidden relative bg-white"
      key={item.id}  // Assuming each course has a unique ID, replace with the actual key
    >
                  {/* image and avatar block */}
                  <div className="cursor-pointer h-48 overflow-hidden">
                    <img
                      src={item?.image?`${userAPI}/images/` + item?.image:''}
                ...
                subCategory
                    ...
                    <div className="flex gap-2 items-center">
                      <DateIcon />
                      <span className="text-sm font-normal">{new Date(item.timestamp).toLocaleDateString()}</span>
                    </div>
                    
                    {/* fix bottom section */}
                    <div className="bottom-2 absolute inset-x-0">
                      <div className="border-t mt-2 mb-2"></div>
                      <div onClick={()=>goDetils(item._id,item.name)} className="text-xl text-blue-500 cursor-pointer pl-4">
                       Enroll Now
                      </div>
